...
const GITHUB_TOKEN ="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"
...
export async function GetRepoByBranchTreeApiRecursive({branchName}){
  const octokit = new Octokit({
    auth: GITHUB_TOKEN
  })
...
let listOfRepos = await octokit.request(`GET /users/${username}/repos?sort=updated&per_page=2`)
